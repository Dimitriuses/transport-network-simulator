...
**Applied to the ladder, every rung that carries conflict is a wall** — `metro-city` −3.494, `towns-and-rail` −5.672, `region` −6.633, against Phase 0's hand-built world at **+0.186** and P1M4's generated tier-3 at **+0.393**. They miss by three to seven rather than by a little, which says the ladder's conflict density is wrong rather than the bar. The realism ceiling is per setting and held throughout — `B-dst-offset` at ±3600 s is something real agencies do — but **`excludes` stops one conflict masking another within a world and nothing stops three of the same setting compounding across operators.** Composed *measurable* consequence has no ceiling the way composed *plausible* consequence does.
