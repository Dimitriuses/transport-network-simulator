...
**And on declared headways Phase 1's exit clause is met at two rungs** (2026-09-11). `metro-city` and `towns-and-rail`, two worlds each from different city seeds: both profiles match at ten seeds on every reference, both ten-seed transfers hold both halves (`competent` +0.028 and +0.039, `tuned` −0.218 and −0.370), and every world screens as a rung. **It is not the end of the city effect** — six `metro-city` cities still span 0.175, and this pair is its two extremes, matched inside a profile noise of about ±0.08 — **and it took one instrument re-run at a resolution chosen before the result**: the three-seed transfer read *not the same tier* on the pair the ten-seed profile called matched. That is one quantity, `competent` on both worlds, decided under two rules (`KNOWN-ISSUES.md` #62, open). **The full gates pass on one world per rung** — 1a, 1b and 2 on both, and Gate 3 at **58 %** and **62 %** of headroom, with `B-time-encoding` the largest conflict in each ablation. **Closing Phase 1 again is a decision the record leaves open**: P1M8 still owes the monotone profile on `LADDER_VERSION` 4, `#47`, and a transfer below `metro-city`.

**Shape is a neutral axis on the current generator**: rail, bus and mixed regions each sit within about 0.04 of the city on every reference. The rail region measured much harder at P1M8 because `B-dst-offset` was still being drawn.
