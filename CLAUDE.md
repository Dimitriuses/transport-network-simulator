...
**A generated world does not currently pass the gates** (`KNOWN-ISSUES.md` #38), and the cause is conflict *placement*: reach-weighting with no bound puts three quarters of the conflicts on the operator carrying two thirds of the network, and a player who ignores that operator outscores one who tries. The network itself is sound — conflict-free, the same player plans most journeys and beats the reference policy. **P1M3 is blocked on the decision.**
