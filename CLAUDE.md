...
**A relaxation whose termination depends on its edge weights must check them.** Three defects now (`KNOWN-ISSUES.md` #40, #44, #45): a non-monotone router, a negative edge from a unit error, and `NaN` arrival times from a deliberately wrong answer key. `NaN` fails every comparison, so the guard that keeps a label set acyclic never fires, and the predecessor walk that reads the answer out never terminates — 21 minutes of CPU and 1.5 GB before it was killed. A time that is not a number is not a time: drop the leg.
...
**A generated world passes all three gates as of P1M2** — 1a solvable, 1b not trivial, 2 discriminating in the right order, 3 at 28 % of headroom. One diagnostic is not clean: the information-set audit flags every solution that plans (`KNOWN-ISSUES.md` #40), which is a bound problem rather than a gate.
