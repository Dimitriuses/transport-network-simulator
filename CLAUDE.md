...
**The gates were ratified against one world and are now asked of several** (`KNOWN-ISSUES.md` #52, #53). Swept across the six-rung ladder that P1M8 then merged, **one rung passed** — `metro-city`, the rung nearest the world Phase 0 built. At the bottom a texture-only rung let a lazy integrator capture 0.796, which is what such a rung is *for*; at the top the journey-time conflict cost came out **negative**, because honest data lets a lazy reader match stops, plan ambitious multi-operator journeys and be taken apart by reality, while conflicted data forces robust single-operator fallbacks. **The merged ladder has not been swept yet** — its top rung is new.
