...
**A `generate` list is drawn as a strength ladder, and not all of them are one** (`KNOWN-ISSUES.md` #48). `_pick` skews towards the strongest listed value by `tier / 5`, so at Tier 5 the last entry wins 92 % of the time — fine for `C-coordinate-offset`'s `[30, 60, 130]`, wrong for `B-time-encoding`'s `epoch_s | epoch_ms | local_naive`, which are three *kinds* of trap rather than three severities. It is why the top of the ladder draws the same conflicts every time, and widening the catalogue does not fix it: the bias operates inside a setting, not across them.
