...
**One setting is the whole distance between the two failures** (`KNOWN-ISSUES.md` #57). The calibrated top rung, measured with `B-dst-offset` switched off on the three operators that drew it and nothing else changed: a lazy integrator goes from **−5.672 to +0.468** — healthier than P1M4's generated tier-3 at +0.393 — and the conflict cost from **502 % to 18 %**, which is *below* Gate 3's 20 % floor. **Both ends fail a gate, and the thirty remaining conflicts cannot reach the middle.** Phase 0's world manages 38 % with fifteen.
