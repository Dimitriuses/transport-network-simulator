...
**The gates were ratified against one world and are now asked of several** (`KNOWN-ISSUES.md` #52, #53). Measured across the calibrated ladder, **one rung of six passes** — `metro-city`, the rung nearest the world Phase 0 built. At the bottom a texture-only rung lets a lazy integrator capture 0.796, which is what such a rung is *for*; at the top **Gate 3 goes negative**, because honest data lets a lazy reader match stops, plan ambitious multi-operator journeys and be taken apart by reality, while conflicted data forces robust single-operator fallbacks. **Gate 3 is only meaningful while the lazy baseline is in a sane range**, and at rungs 4-5 it captures −5.7 and −6.6.

**A rung carries its city, and its operators are generated** (P1M6). `LADDER` declares arms, sites per arm, hub quays, chords, regional and metro lines, the operator roster by *role*, and the reach cap — which scales with the roster, because two operators cannot both be under a half. Worlds run from 32 sites and 2 operators at `small-town` to 120 sites and 6 at `region`. **The ids are generated from the world's seed**, so two worlds of a rung share none of them, and `tuned` drops a feed its key has no entry for rather than falling back to inference.
