...
**Status 2026-09-06 — two of the three clauses are done and the first one fails.**

* ✅ **The profile exists** (`#24`). `npm run profile` reports what each reference solution achieves, as a vector; the tolerance is the world's own seed-to-seed spread, measured from the same runs rather than assumed.
* ✅ **The clearance bar survives a change of scale**, as a position between named references.
* ❌ **Two same-tier worlds do not match** (`#42`). `naive` differs by 2.0× the noise and `competent` by 1.6×, while `null` and `blind` agree — the two references that actually integrate are the two that disagree. The generator samples a catalogue at a declared density, which produces a *distribution* of difficulties rather than a difficulty.
* ⬜ **A solution transferring between worlds** is untouched, and remains the harder half.
