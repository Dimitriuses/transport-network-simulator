...
**Delivered.** `--shape polycentric` builds a region: three towns, each generated by the same code and then namespaced, **each with its own bus company and its own tram**, joined by a railway. A tier-4 region is 88 sites, 101 quays, 23 lines and **8 operators**, of which the largest serves 18 % of the network.
