...
**The bottom of the ladder had no variety, and fixing it found something larger** (`KNOWN-ISSUES.md` #43, fixed). Tier 1's quota asked for two settings from a section holding exactly two, so every Tier 1 world was the same world — the same shape as `#32`, a tier satisfying "two worlds of a tier are equally hard" vacuously. Two cosmetic settings were added and Tier 1 now varies.

**What that uncovered is the risk worth carrying forward: a tier's quota counted settings, and cosmetic settings are settings.** Adding texture cost tiers 2 and 3 more than a whole semantic conflict per world, silently, with every instrument still reporting the same tier. That is `#42`'s own defect — composition not actually fixed — surviving inside `#42`'s fix. The quota now buys semantic settings first and texture is drawn outside it, so **the size of the cosmetic pool no longer affects difficulty**.

**The consequence is that generated worlds are harder than the numbers on this page.** Semantic content at tiers 2 and 3 rose by about two conflicts per world when the quota started meaning what it declared. Every difficulty figure recorded against a generated world — P1M2's 28 % of headroom, P1M3's 22 %, P1M4's cal-a/cal-b profiles and transfer figures — was measured on the older generator.

**Re-measured once, on a fresh tier-3 world:** all three gates still pass, and Gate 3 reads **31 % of headroom** against P1M3's 22 % and P1M2's 28 %. Headroom itself rose from 10.85m to 12.12m, the ambiguity floor is unchanged at 1 %, and a lazy integrator captures 0.393. **One world is one draw** — `npm run calibrate:tier` is what says where the tier's middle now sits, and the cal-a/cal-b pair should be rebuilt before the transfer figures are quoted again.

**Two rungs still rest on the weaker guarantee** (`#47`): section B holds one setting and every tier from 2 up draws it, and Tier 5 draws all three of section D. Neither produces identical worlds today, because their values differ substantially — but a choice of values is weaker than a choice of settings, which is exactly what Tier 1 demonstrated.
