...
### P2M3 — The view that explains it — **delivered 2026-09-14**
...
**Decided 2026-09-14, on measurements taken first:** a local page served by `npm run view` with no dependencies, which regenerates response bodies on demand; run logs streamed to a partial file and written canonically at the end, with `verbatim` and its cap, and SQLite compaction deferred because a whole day's log is 0.3–0.9 MB; a knowledge band of four marks read from the log — announced, knowable, the player's first read of that feed, its warning — ending at the decision point, where "read" means *had it in hand* rather than understood it; and all three disclosure levels, set on the run, with `attributed`'s section costs from a per-player counterfactual, `npm run attribute`.

**Delivered 2026-09-14 — the exit met, and held by a test rather than by eye.** `npm run demo` writes `runs/<name>.ndjson` and prints the command that explains it. The page has an overview (scorecard, where capture went, travellers ranked by what they lost), a traveller timeline with the band beneath it and a sentence for every break, replan, band and outcome, an API request view with each obligation's calls and their regenerated bodies, and map replay at `full`. **Movements are regenerated by replaying the run**, never logged, and a replay that decides any traveller differently from the log is refused. `src/viewer/test/timeline.test.ts` requires every traveller's steps to reproduce its recorded journey, wait and transfers, the timelines together to give back the scorecard's capture and Information counts in open and closed loop, and no canonical quay id to survive below `full`.

**What it does not do:** passenger flows are the scored travellers only, because there is no background population until P2M6; and *enjoyable to work against*, the phase exit's other half, is the playtest's to measure.

**Building it found four defects, and the view found them faster than the tests before it had** — `OBSERVABILITY.md` §9's prediction. The simulator never sent `traceparent` (`#69`, fixed). A traveller who did not arrive was recorded with the wait and transfers of an earlier moment (`#70`, fixed, capture unchanged). Below `full`, canonical quay ids leaked through failure reasons and attribution buckets (fixed in the viewer). And **the Information family counts events no warning could have served, at a decision point placed at the journey's first stop rather than the traveller's** — 41–52 % of events on both worlds are knowable only after their recorded decision point (`#71`, open, needs a decision). The first traveller drawn also showed `#68` on screen: a non-arrival scored better than the best announced route.
