...
*Re-measured at P2M0, on a router whose walk transfers are no longer rationed by a budget of rides (`KNOWN-ISSUES.md` #40): 6.19m reachable of **8.48m**, conflicts **2.98m at 35 %**, all gates still passing. The figures above are what Phase 0 exited on and are kept as recorded. *Re-measured again on 2026-09-13, after the ablation began comparing matched populations (`KNOWN-ISSUES.md` #56): conflicts **3.10m at 38 %** of the 8.12m headroom on the 82 of 98 journeys both runs planned; 1a, 1b and Gate 2 unchanged.*
...
*And amended a third time, 2026-09-11* (`KNOWN-ISSUES.md` #48, #59, #61). **Two worlds of one tier share their city.** Generated from different seeds they have one graph, one roster, one share of it per operator and one timetable; the seed moves some sites, renames every operator and draws the conflicts. *As first written this was false in one respect: the seed also drew each radial line's headway, and that timetable moved a rung's difficulty more than its conflicts did (`#61`). The generator was corrected, not the clause.* So the tasks are non-memorisable in their *conflict draw*, and that is the claim this clause makes — not that the cities differ.

The overfitted solution is therefore keyed by what survives a rename — **operator kind and rank** (`@tns/schema` `operatorKeys`) — not by operator id. Keyed by id it read no feed on any other world, and `npm run transfer` certified two worlds identical except for their operators' names: the second clause, satisfied by the names alone. Keyed by kind and rank, the same pair reads *too alike*, which is the right answer. The clause is measured on single-centre rungs, since a polycentric region's symmetric towns cannot be keyed at all.
