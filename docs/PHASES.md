...
> **Reopened on 2026-09-08.** `KNOWN-ISSUES.md` #48: a tier declares which conflicts a world holds and nothing else, so every generated world at every tier is the same city with the same three operators under the same names. The completion clause below was met at one rung on that generator; the ladder is being given a second dimension — **scale as the ordered tier, shape as a declared axis** — and the exit re-established across rungs. `P1M5`–`P1M8` in [`../ROADMAP.md`](../ROADMAP.md); Phase 2 pauses behind them, as Phase 0 paused behind P1M0 in 2026-09-02.
