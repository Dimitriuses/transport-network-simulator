...
* **Issuance stays sequential.** Measured on the committed world and on a generated `metro-city` world, a reference player answers a plan or replan in 1–7 ms and a tick in 13–76 ms, against gaps of one simulated second or more between distinct obligations; at 60× the slowest tick still costs under five simulated seconds. Issuing concurrently and applying responses in `request_id` order (contract §9.1) was expected to be needed by the closed loop; P2M2 built it sequentially and did not need it — at every traveller an app user, the committed world asks 127 plans and replans over the day — so it waits for a background population (P2M6), which is what multiplies request volume.
...
* **In open loop only a plan's deadline can bind in wall time; in closed loop a replan's can too.** Open loop walks a traveller's whole journey when its plan is answered, so each replan is asked then, with an `issued_at` hours ahead of τ, and its deadline lies in the simulated future (`KNOWN-ISSUES.md` #66, kept in open loop by decision). Closed loop (P2M2) queues the replan at the break and paces it like any obligation. **Measured at 600× on the committed world with `competent`:** in open loop none of 37 replans carried a lag or could time out; in closed loop all 23 were paced, the latest issued 20 simulated seconds late, and that one missed its deadline with no wall budget left — a late issuance, not a slow player, whose replans took 2–4 ms. The same runs timed out 33 and 20 plans, because at 600× a twenty-second deadline is 33 wall milliseconds: which answers land at that speed belongs to the machine.
