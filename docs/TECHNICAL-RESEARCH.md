...
* **Transcendental functions are the real hazard.** ECMA-262 only *recommends* fdlibm for `Math.sin`/`cos`/`tan`; it does not require it, and specifies results as implementation-approximated. In practice V8 statically links its own routines (bundled llvm-libc, fdlibm-derived for sin/cos), so results are consistent across operating systems for a given V8 — but **not guaranteed across V8 versions**. A concrete recent example: `Math.tanh` was changed in V8 14.8.57 / Chrome 148 to call `std::tanh`, which reads the host libm, making it OS-dependent where it previously was not.
  * **Mitigation (recommended):** keep transcendentals out of the simulation core entirely. Geodesic distances are the only real need, and they can be computed once at world-build time. IEEE-754 `+ - * /` and `sqrt` are exactly specified and reproduce everywhere; only the transcendental library is unstable.
  * **Correction, found at P0M2: moving the computation to Python does not make it deterministic — it relocates the problem.** CPython's `math.sin`/`cos`/`asin` delegate to the platform libm, so glibc on Linux and UCRT on Windows can differ in the last ULP exactly as two V8 versions can. The offline pipeline is not a magic zone where floating point behaves; it is simply a place where the instability happens *once* instead of every run.
  * **What actually fixes it:** never store a raw transcendental result. Round it to a precision far coarser than libm noise before it enters the world bundle. Distances are stored as **integer metres**, which puts roughly nine orders of magnitude between the noise (~10⁻¹⁴ m) and the stored value. The bundle then contains no float that any libm produced — only source-literal coordinates and derived integers.
...
Briefly: P0M0 scaffolding and the lint rules the specs demand; P0M1 a walking skeleton crossing every layer, to retire the risk that eight specs written before any code do not fit together; P0M2 the oracle and baselines, load-bearing for both the reference policy and the entire scoring normalisation; then conflicts, the live world, judgement, and the reference player plus conformance suite.
